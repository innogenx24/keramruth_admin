...
const OrderManagement = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Filter pending orders where higher_role_id === 1 and sort by createdAt in descending order
      const sortedPendingOrders = allOrders
        .filter(
          (order) => order.status === "Pending" && order.higher_role_id === 1
        )
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

      setPendingOrders(sortedPendingOrders);

      // Sort completed orders by updatedAt in descending order (all accepted/cancelled orders)
      const sortedCompletedOrders = allOrders
        .filter(
          (order) => order.status === "Accepted" || order.status === "Cancelled"
        )
  ...
  const handleAction = async (orderId, action) => {
    ...
    try {
      ...
      fetchOrders();
      setSnackbarMessage(
        action === "accept" ? "Order accepted successfully" :
        action === "reject" ? "Order rejected successfully" :
        action === "modify" ? "Order modification requested" :
  ...
  const renderPagination = (page, setPage, orders) => {
    ...
    const canGoNext = totalFilteredRows > rowsPerPage;
        ...
        >
          <Table stickyHeader aria-label={`${title} Table`}>
            ...
            <TableBody>
              {totalFilteredRows === 0 ? (
                <TableRow>
                  <TableCell colSpan={isActionable ? 8 : 9} align="center">
                    No {title.toLowerCase()} available
                  </TableCell>
                </TableRow>
              ) : (
  ...
  return (
    <div>
      ...
      {/* Modify Order Dialog */}
      <Dialog open={modifyDialogOpen} onClose={closeModifyDialog}>
          ...
          {selectedOrder && (
            <Box sx={{ mt: 2 }}>
              <Typography variant="subtitle1">
                Order ID: {selectedOrder.orderUniqueId}
              </Typography>
              <Typography variant="subtitle1">
        ...
        <DialogActions>
          <Button onClick={closeModifyDialog} color="primary">
            Cancel
          </Button>
          <Button
            onClick={() => {
              handleAction(selectedOrder?.orderId, "modify");
              closeModifyDialog();
            }}
            color="warning"
            variant="contained"
