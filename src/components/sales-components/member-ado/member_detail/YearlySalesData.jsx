    ...
    const fetchSalesAchievement = async () => {
      try {
        const response = await axios.get(
          `${API_END_POINT}/user_sales_detail/yearly_sales_achievement/${roleToUse}/${memberID}`
        );
        const { yearlyDetails } = response.data; // Use yearlyDetails instead of monthlyDetails
        setSalesData(yearlyDetails || []); // Set the yearly details to the state
        setLoading(false);
      } catch (error) {
                  ...
                  <Box display="flex" justifyContent="space-between" gap={2}>
                    ...
                    >
                      <Box position="relative" display="inline-flex" mr={2}>
                        ...
                        <CircularProgress
                          variant="determinate"
                          value={salesData.achievementAmountPercent}
                          size={80}
                          thickness={5}
                          style={{
                            position: "absolute",
                            color:
                            salesData.achievementAmountPercent < 50
                                ? "red"
                                : salesData.achievementAmountPercent < 75
                                  ? "orange"
                                  : "green",
                          }}
                        />
                        ...
                        >
                          ...
                          >
                            {(parseFloat(salesData.achievementAmountPercent) || 0).toFixed(2)}%
                          </Typography>
                        </Box>
                      </Box>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Rs. {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(salesData.MonthlyTargetAmount) || 0)}
                        </Typography>

                        <Typography variant="body2" sx={{ color: "green", fontWeight: "bold" }}>
                          Achieved: Rs. {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(salesData.AchievementAmount) || 0)}
                        </Typography>

                        <Typography variant="body2" color="error" fontWeight="bold">
                          Pending: Rs. {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(salesData.pendingAmount) || 0)}
                        </Typography>
                    ...
                    >
                      <Box position="relative" display="inline-flex" mr={2}>
                        ...
                        <CircularProgress
                          variant="determinate"
                          value={salesData.StockAchievementPercent} // Keep as is, since it's already in decimal form
                          size={80}
                          thickness={5}
                          style={{
                            position: "absolute",
                            color:
                            salesData.StockAchievementPercent < 50
                                ? "red"
                                : salesData.StockAchievementPercent < 75
                                  ? "orange"
                                  : "green",
                          }}
                        />
                        ...
                        >
                          <Typography variant="h6" fontWeight="bold" sx={{ fontSize: "1.08rem !important", color: "primary" }}>
                            {(parseFloat(salesData.StockAchievementPercent) || 0).toFixed(2)}% {/* Safely format */}
                          </Typography>
                        </Box>
                      </Box>
                      <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Stock: {new Intl.NumberFormat('en-IN').format(parseFloat(salesData.StockTarget) || 0)}
                        </Typography>

                        <Typography variant="body2" sx={{ color: "green", fontWeight: "bold" }}>
                          Stock Achieved: {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(salesData.StockAchievement) || 0)}
                        </Typography>

                        <Typography variant="body2" color="error" fontWeight="bold">
                          Stock Pending: {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(salesData.PendingStockTarget) || 0)}
                        </Typography>
...
export default YearlySalesData